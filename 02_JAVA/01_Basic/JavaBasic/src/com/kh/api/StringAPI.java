...
public class StringAPI {
	...
	public void method02() {
		...
		System.out.println("ch : " + ch); //W
		
		//문자열.contains(찾고자하는 문자열) : boolean
		//문자열에 전달된 문자열이 포함되어 있는지 확인하여 결과를 반환
		System.out.println("str1에 ello라는 문자열이 있는지 확인 : " + str1.contains("ello"));
		System.out.println("str1에 ello라는 문자열이 있는지 확인 : " + str1.contains("bye"));
		
		//문자열.concat(String str) : String
		//문자열과 전달된 또 다른 문자열을 하나로 합쳐서 새로운 문자열을 리턴
		String str2 = str1.concat("!!!");
		System.out.println("str2 : " + str2);
		System.out.println("str2 : " + str1 + "!!!");
		System.out.println("str2 : " + new StringBuilder(str1).append("!!!").toString());
		
		//문자열.substirng(시작위치, [끝위치])
	...
	public void method03() {
		...
		System.out.println(st.nextToken());
		System.out.println(st.nextToken());
//		System.out.println(st.nextToken()); 더이상 토큰이 남아있지 않을때 다음토큰 호출시 예외발생
		
		System.out.println();
		
		st = new StringTokenizer(str, ",");
		int size = st.countTokens(); //남은 토큰의 개수
		...
		st = new StringTokenizer(str, ",");
		while(st.hasMoreElements()) { //남은 토큰이 존재하는가?
			System.out.println(st.nextToken());
		}
