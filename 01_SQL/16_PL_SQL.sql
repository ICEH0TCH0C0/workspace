...
/*
    레퍼런스타입 변수로 EID, ENAME, JCODE, SAL, DTITLE을 선언하고
        ...
        DEPARTMENT(DEPT_TITLE)을 참조하여 사용자가 입력한 사번의 사번, 사원명, 직급코드,
        급여, 부서명 조회후 각 변수에 담아서 출력
*/
...
-- 보너스를 받지 않은 사원은 보너스를 지급받지 않은 사원입니다.
-- 보너스를 받지 않은 보너스 : XXX 출력
DECLARE
    EID EMPLOYEE.EMP_ID%TYPE; 
    ENAME EMPLOYEE.EMP_NAME%TYPE;
    JCODE EMPLOYEE.JOB_CODE%TYPE;
...
BEGIN
    ...
    IF BONUS = 0 THEN
        DBMS_OUTPUT.PUT_LINE('보너스를 받지 않은 사람입니다.');
    END IF;
...
--레퍼런스 변수 (EID, ENAME, DTITLE, NCODE)를 생성하고
--각 테이블에서 EMP_IDM EMP_NAME, DEPT_TITLE, NATIONAL_CODE를 참조하라
--일반타입변수 TEAM을 문자열타입으로 생성하고 사용자가 입력한 사원정보를 가져와서
--사번, 이름, 부서명, 근무국가코드 조회 후에 각 변수에 대입.
--NCODE값이 KO일경우 -> TEAM = 국내팀
...
--I에 1부터 10인 순서를 부여해서 각각 반복문 내의 코드를 실행
BEGIN
    FOR I IN 1..10
    LOOP
        DBMS_OUTPUT.PUT_LINE(I);
...
--I에 10부터 1인 역순으로 부여해서 각각 반복문 내의 코드를 실행
BEGIN
