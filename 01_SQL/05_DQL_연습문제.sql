...
SELECT EMP_NAME, ABS(TRUNC(HIRE_DATE - SYSDATE, 0)) AS 근무일수1, TRUNC(SYSDATE - HIRE_DATE, 0) AS 근무일수2 
FROM EMPLOYEE;
...
SELECT EMP_NAME, DEPT_CODE, TO_CHAR(TO_DATE(SUBSTR(EMP_NO, 1, 6), 'RRMMDD'), 'YY"년"MM"월"DD"일"') FROM EMPLOYEE;
...
SELECT EMP_NAME, SUBSTR(EMP_NO, 0, 6) 앞자리, SUBSTR(EMP_NO, 8) 뒷자리, SUBSTR(EMP_NO, 0, 6) + SUBSTR(EMP_NO, 8) 합 
FROM EMPLOYEE
WHERE EMP_ID = '201';
...
SELECT COUNT(*) AS "전체 직원 수",
    ...
    COUNT(DECODE(EXTRACT(YEAR FROM HIRE_DATE), 2009, 1, NULL)) AS "2009년"
FROM EMPLOYEE;
