 ...
 * */
@WebServlet("/gettest.do") //mapping에 대한 설정
public class RequestGetServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		//get방식으로 요청시 doGet메서드를 실행하여 요청을 처리해준다.(톰캣이 서블릿객체를 생성해서 메서드 호출까지 해줌)
		 ...
		 * requset의 parameter 영역안에 전달된 값을 추출
		...
		//1 응답으로 출력할 내용은 html이고 문자셋은 utf-8로 선언
		response.setContentType("text/html; charset=utf-8");
		
		//2 응답받는 사용자와의 스트임을 연결
		...
		out.printf("<span>%s에 살고 있습니다.</span><br>", address);
		out.print("성별은 ");
		if(gender == null) {
			out.println("미입력상태입니다.");
