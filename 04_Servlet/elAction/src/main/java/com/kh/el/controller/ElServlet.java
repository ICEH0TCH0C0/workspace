...
public class ElServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 * 	애플리케이션(프로젝트) 당 1개 존재하는 객체, 전역에 공유, 서버 재시작 전까지 유지.
		 * 	멀티스레드 환경에서 공유되므로 동시성 주의
		 * 
		 * 2. HttpSession (session scope)
		 *  브라우저(사용자) 당 1개 존재하는 객체, 로그인 상태/ 사용자별 데이터를 저장하는 것에 적합
		 *  이 영역에 데이터를 담으면 jsp/servlet단에서 사용이 가능
		 *  
		 * 3. HttpSerbletRequest (request scope)
		 *  요청마다 1개 존재하는 객체
		 *  이 영역에 데이터를 담으면, request객체를 포워딩받는 응답 jsp까지 사용 가능
		 * 
		 * 4. pageContext (page scope)
		 * 	jsp 페이지마다 존재하는 객체, 각장 좁은 범위 객체(내부 변수 처럼 생각)
		 * 	
		 * 위 객체의 공통 API :
		...
		session.setAttribute("lecture", new Person("홍길동", 22, "여자"));
		
		//3. request scope
		request.setAttribute("scope", "request");
		request.setAttribute("studnet", new Person("홍길서", 20, "남자"));
