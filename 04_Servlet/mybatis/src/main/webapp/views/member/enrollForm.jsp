...
<body>
        ...
        <div class="form-container">
            ...
            <form id="enroll-form" action="${pageContext.request.contextPath}/insert.me" method="post">
                <div class="row form-row">
                    ...
                    <div class="col-md-4">
                        <button type="button" class="btn btn-outline-primary w-100" onclick="idDulpivateCheck()">중복확인</button>
                    </div>
                </div>
    ...
    <script>
        ...
        */
        ...
        //존재하지 않는다면, 사용가능 -> alert 메시지 출력(사용가능한 아이디입니다) -> 정말 사용할거야? yes -> 더이상 변경 x
        //                                                                                        no -> 다시 입력
        
        function idDulpivateCheck() {
            const idInput = document.querySelector("#enroll-form input[name=userId]");

            if(idInput.value.length < 5){ // 아이디 형식에 대한 예외처리
                return
            } 

            $.ajax({
                url: "idDulpivateCheck.me",
                type: "get", //기본값 get
                data: {
